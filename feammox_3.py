...
NH4_input = st. number_input("NH₄⁺ concentration [mmol/L]", min_value=0.0, max_value=10.0, value=2.0, step=0.001) # [mmol/L]
NH4 = NH4_input * 10**(-3) # changing [mg/L] in [g/L]
R = 0.008314 #kJ mol^-1
T = 297.15 #Kelvin
...
# calculation of ΔG
st.write(f"ΔG for pH = {pH} and NH₄⁺ = {NH4_input} mmol/L is **{func_G_r(pH, NH4):.2f} kJ/mol**")


#--------------------- figures ------------------------------------
# Generation of x values
pH_range = np.arange(1, 14, 0.1)
NH4_range_mmol = np.logspace(-3, 1, 200)# von 0.001 mmol/L bis 10 mmol/L, 200 Punkte

G_r_values_NH4 = [func_G_r(pH, NH4*10**-3) for NH4 in NH4_range_mmol]
...
fig2.add_trace(go.Scatter(
    ...
    hovertemplate='NH₄⁺: %{x:.4f} mmol/L<br>ΔG: %{y:.2f} kJ/mol'
))
